...
class Json_base():
	...
	@classmethod
	def load_object(cls, data_identifier, json_dictionary = None, file_path = None, cache_path = None):
		try:
			...
			if (json_dictionary == None):
				...
				for key, value in loaded_object.prepare_load(loaded_dict = json_dictionary).items():
	...
	def __str__(self):
		if type(self).__name__ is not Json_base.__name__:
			return "This is " + type(self).__name__ + " which is a child of the Json_base class and has not implemented the __str__() function yet"
		else:
	...
	def __repr__(self):
		if type(self).__name__ is not Json_base.__name__:
			return "This is " + type(self).__name__ + " which is a child of the Json_base class and has not implemented the __repr__() function yet"
		else:
	...
	def prepare_store(self):
		if type(self).__name__ is not Json_base.__name__:
			return "This is " + type(self).__name__ + " which is a child of the Json_base class and has not implemented the prepare_store() function yet"
		else:
	...
	def prepare_load(self, loaded_dict = {}):
		if type(self).__name__ is not Json_base.__name__:
			return "This is " + type(self).__name__ + " which is a child of the Json_base class and has not implemented the prepare_load() function yet"
		else:
	...
	def store(self, data_identifier, file_path = None, cache_path = None):
		try:
			if file_path is None and cache_path is None:
				...
				target_path = os.path.join(type(self).file_prefix, data_identifier + type(self).file_suffix)
				target_file = open(target_path, mode='w', encoding='UTF8')
				target_file.write(json.dumps(self.prepare_store(), cls = self.JSON_Encoder))
				target_file.close()
			...
			# handle any IO issues
			print("ERROR "+ type(self).__name__+ " STORE FAILED - Implement error handling")
			#
	...
	def load(self, data_identifier, file_path = None, cache_path = None):
		try:
			if file_path is None and cache_path is None:
				...
				for key, value in self.prepare_load(loaded_dict = json.loads(target_file.readline())).items():
			...
			# handle any IO issues
			print("ERROR "+ type(self).__name__+ " LOAD FAILED - Implement error handling")
			#
			# === IMPLEMENT CACHE ERROR HANDLING HERE === 
			# 
			# === IMPLEMENT CACHE ERROR HANDLING HERE === 
	...
	def update(self):
		if type(self).__name__ is not Json_base.__name__:
			print("This is " + type(self).__name__ + " which is a child of the Json_base class and has not implemented the update() function yet")
		else:
