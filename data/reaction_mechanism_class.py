...
class reaction_mechanism(json_base_class.Json_base):
    ...
    def test():
        ...
        if(test_rxn == new_rxn):
            print("The reaction_mechanism was successfully stored and loaded, no data was lost")
            return True
        else:
            print("There was an error storing and loading the reaction_mechanism, the objects are not identical")
    ...
    def __init__(self, num_rxn, num_species, mol_list, rxn_array, reagEnrg, *args, **kw_args):
        # handle molecule JSON serialization properly
        self.JSON_Encoder = molecular_species.mol_JSON_Encoder

        self.number_of_reactions            = int(num_rxn)          # number of columns in self.coefficient_array
        self.number_of_species              = int(num_species)      # number of rows in self.coefficient_array ALSO the length of self.molecular_species_dict 
        ...
        self.molecular_species_dict         = dict([(species_name, molecular_species.molecular_species(species_name, reagEnrg[species_name])) for species_name in mol_list]) 
        # the r, p, v matrices
    ...
    def __eq__(self, other):
        return (isinstance(other, reaction_mechanism)           
            ...
            and np.all(self.reactant_coefficient_array     == other.reactant_coefficient_array)    
            and np.all(self.product_coefficient_array      == other.product_coefficient_array)                 
    ...
    def prepare_store(self):
        prepared_dict =  {  'number_of_reactions'     : self.number_of_reactions, 
    ...
    def prepare_load(self, loaded_dict = {}):
        ...
        loaded_dict['molecular_species_dict']  = dict([(species_name, molecular_species.molecular_species.load_object(species_name)) for species_name in loaded_dict['coefficient_dict'].keys()]) 
        loaded_dict['coefficient_array']       = np.array(loaded_dict['coefficient_array'], dtype = float)
        return loaded_dict
    ...
    def update(self):
        ...
        print("Num species", self.number_of_species, '\n',
            ...
            '\n')
        '''
        assert(self.number_of_species     == len(self.molecular_species_dict) == len(self.coefficient_dict))
        ...
        # do the work
        self.reactant_coefficient_array = np.select([self.coefficient_array < 0.0, True], [np.fabs(self.coefficient_array), 0.0]) 
        self.product_coefficient_array  = np.select([self.coefficient_array > 0.0, True], [self.coefficient_array, 0.0])
