...
def run_true_experiment(puzzle, condition, condition_path, progress_tick):
	...
	for reagent_name, reagent_obj in puzzle.reagent_dict.items():
		stream.write("                Pre-equilibrate",reagent_name,":")
	...
	stream.write("            Pre-equilibration finished -- now all reagents are under pre-equilibrium.")
	...
	#stream.write(os.path.join(condition_path, "plotData_t_"))
...
def run_proposed_experiment(puzzle, condition, condition_path, progress_tick, solution, solution_path, written_true_data = None):
	...
	try:
		...
		#stream.write("Rate Constants " + str(rate_consants))
	...
	stream.write(str(proposed_model.rate_constant_array))
