...
def run_true_experiment(puzzle, condition, condition_path, progress_tick, stream = sys.stdout):
	def show_all_concentrations():
		logger.info("                    Update the concentration record of the whole lab:")
		buff = '                        '
	...
	for reagent_name, reagent_obj in puzzle.reagent_dict.items():
		...
		if reagent_obj.number_of_species is 1:
			...
			logger.info("                    This reagent only has one species so no pre-equilibration happened.")
			logger.info("                    This reagent had a concentration of "+ str(condition.reagent_concentrations[reagent_name]))
		...
		logger.info("			* reagent_obj.coefficient_dict: "+str(reagent_obj.coefficient_dict))
		logger.info("			* reagent_conc_array -- before: "+str(reagent_conc_array))

		equilibrate(pre_equil_model, progress_tick, diag = False)
		logger.info("			* reagent_conc_array -- after: "+str(pre_equil_model.reaction_profile[-1]))
		...
		for name, index in reagent_obj.coefficient_dict.items():
			...
			if np.isnan(concentration_of_this_species):
				logger.info('                    [Warning] Concentration of '+name+' is NaN. Falling back to un-pre-equilibrated concentrations. This is a bug.')
	...
	logger.info("            Pre-equilibration finished -- now all reagents are under pre-equilibrium.")
	...
	true_model = experiment_class.experiment(puzzle, condition.reaction_temperature, input_time = [0.0], rxn_profile = temp)
	...
	logger.info("            Now we can finally let the actual reaction happen -- let's pour everything into the beaker:")
...
def run_proposed_experiment(puzzle, condition, condition_path, progress_tick, solution, solution_path, written_true_data = None, stream = sys.stdout):
	...
	proposed_model = experiment_class.experiment(solution, condition.reaction_temperature, input_time=data[0][:], rxn_profile=np.swapaxes(data[1::1][:], 0, 1))
	...
	written_data = np.transpose(np.column_stack([proposed_model.time_array, proposed_model.reaction_profile])) #input_output.write_ODE(condition.reaction_temperature, proposed_model.time_array, proposed_model.reaction_profile, data_file_name = os.path.join(solution_path, "plotData_t_"))
