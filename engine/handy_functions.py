...
def file_error(filename, action):
	print("ERROR could not " + str(action) + " file: \'" + str(filename) + "\'", file = sys.stderr)
...
def test():
	...
	logging.config.fileConfig('logging.conf') # apprently this method is outdated though
	...
	# done
	logging.basicConfig(format='%(asctime)s %(message)s', datefmt='%m/%d/%Y %I:%M: %p') # dont need seconds
